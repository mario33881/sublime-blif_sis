...
def write_key(t_key, t_delay):
    """
    Write the <t_key> char and then wait <t_delay> (if Sublime Text window is focused).
    
    :param string t_key: char with a keyboard key
    :param (float, int) t_delay: seconds to wait after the char is typed
    """
    if "Sublime Text" in aw.get_active_window():
        time.sleep(t_delay)
...
def write_hotkey(t_key, t_delay):
    """
    Execute <t_key> hotkey and then wait <t_delay> seconds (if Sublime Text window is focused).
...
def test_fsm():
    ...
    """
    # start from template
    write_word("sis_fsm", delay)
    write_hotkey("enter", delay)
...
def delete_file_content(t_delay):
    """
    Deletes a file content (if Sublime Text window is focused).
    :param (float, int) t_delay: seconds to wait after each char is typed
    """
    if "Sublime Text" in aw.get_active_window():
        # click CTRL + A
    ...
    if template_name == "all":
        ...
        for test in tests:
            if "Sublime Text" in aw.get_active_window():
                ...
                print("[TEST] ERROR: no focus on the Sublime Text window during '{}' test".format(test["template_name"]))
    else:
        found = False
        n = 0
        while n < len(tests) and not found:
            ...
            if test["template_name"] == template_name:
                if "Sublime Text" in aw.get_active_window():
                    ...
                    print("[TEST] ERROR: no focus on the Sublime Text window during '{}' test".format(template_name))
                found = True
