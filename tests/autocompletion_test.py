...
def test_latch():
    ...
    tt.write_hotkey("tab", delay)
    tt.write_word("INPUT", delay)
    tt.write_hotkey("tab", delay)
	tt.write_word("OUTPUT", delay)
    tt.write_hotkey("tab", delay)
	tt.write_word("re", delay)
    tt.write_hotkey("tab", delay)
	tt.write_word("NIL", delay)
    tt.write_hotkey("tab", delay)
	tt.write_word("0", delay)
    tt.write_hotkey("tab", delay)
    tt.write_word("rest of file should go here", delay)
    ...
    if keyword_name == "all":
        ...
        for test in tests:
            if "Sublime Text" in aw.get_active_window():
                ...
                print("ERROR: no focus on the Sublime Text window during '{}' test".format(test["keyword_name"]))
    else:
        found = False
        n = 0
        while n < len(tests) and not found:
            ...
            if test["keyword_name"] == keyword_name:
                if "Sublime Text" in aw.get_active_window():
                    ...
                    print("ERROR: no focus on the Sublime Text window during '{}' test".format(keyword_name))
                found = True
